...
import explicit.MDP.DoubleTransitionToDoubleFunction;
...
public interface DTMC<Value> extends Model<Value>
{
	 ...
	 */
	@FunctionalInterface
	public interface TransitionToDoubleFunction<Value> {
	 ...
	 * @param c the consumer
	 */
	public default Value sumOverTransitions(final int s, final TransitionToDoubleFunction<Value> f)
	{
		class Sum {
	 ...
	 */
	@FunctionalInterface
	public interface DoubleTransitionConsumer<Value> {
	 ...
	 * @param c the consumer
	 */
	public default void forEachDoubleTransition(int s, DoubleTransitionConsumer<Value> c)
	{
		for (Iterator<Entry<Integer, Value>> it = getTransitionsIterator(s); it.hasNext(); ) {
			Entry<Integer, Value> e = it.next();
	...
	public default double mvMultJacSingle(int s, double vect[])
	{
		class Jacobi {
			...
			void accept(int s, int t, Value prob) {
				if (t != s) {
					d += ((Double) prob) * vect[t];
				} else {
					diag -= ((Double) prob);
				}
			}
		...
		for (OfInt it = states.iterator(); it.hasNext();) {
			...
			forEachTransition(state, (s, t, prob) -> {
				if (s != t) {
					...
					result[t] += deltaT * ((Double) prob) * vect[s];
				}
			});
		}
	}
